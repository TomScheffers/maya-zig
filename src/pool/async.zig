...
const Task = struct {
    ...
    fn onSchedule(tp_task: *ThreadPool.Task) void {
        const task: Task = @fieldParentPtr("tp_task", tp_task);
        resume task.frame;
    }
    ...
    return struct {
        ...
        pub fn complete(self: *Self, value: T) void {
            ...
            suspend {
                ...
                if (state != 0) {
                    ...
                    if (state != DETACHED) {
                        const joiner: *Waiter = @ptrFromInt(state);
        ...
        pub fn join(self: *Self) T {
            ...
            suspend {
                ...
                if (@as(?*Waiter, @ptrFromInt(self.state.swap(@intFromPtr(&waiter), .AcqRel)))) |completer| {
        ...
        pub fn detach(self: *Self) void {
            ...
            if (@as(?*Waiter, @ptrFromInt(?self.state.swap(DETACHED, .Acquire)))) |completer| {
                completer.task.schedule();
            }
        }
    };
